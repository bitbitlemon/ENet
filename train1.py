...
class ENet(nn.Module):
    ...
    def forward(self, x):
        ...
        print(f'After InitialBlock: {x.shape}')

        x, max_indices1 = self.downsample1_0(x)
        print(f'After DownsamplingBottleneck 1: {x.shape}')

        x = self.regular1_1(x)
        x = self.regular1_2(x)
        x = self.regular1_3(x)
        x = self.regular1_4(x)
        print(f'After RegularBottleneck 1: {x.shape}')

        x, max_indices2 = self.downsample2_0(x)
        print(f'After DownsamplingBottleneck 2: {x.shape}')
        ...
        x = self.dilated2_8(x)
        print(f'After Bottlenecks 2: {x.shape}')

        x = self.upsample4_0(x, max_indices2, output_size=(64, 64))
        print(f'After UpsamplingBottleneck 4: {x.shape}')

        x = self.regular4_1(x)
        x = self.regular4_2(x)
        print(f'After RegularBottleneck 4: {x.shape}')

        x = self.upsample5_0(x, max_indices1, output_size=(128, 128))
        print(f'After UpsamplingBottleneck 5: {x.shape}')

        x = self.regular5_1(x)
        x = self.fullconv(x)
        print(f'After FullConv: {x.shape}')
